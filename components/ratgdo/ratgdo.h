...
namespace ratgdo {
    ...
    */
    ...
    public:
        ...
        void subscribe_rolling_code_counter(std::function<void(uint32_t)>&& f) { this->rollingCodeCounter.subscribe(std::move(f)); }
        void subscribe_opening_duration(std::function<void(float)>&& f) { this->openingDuration.subscribe(std::move(f)); }
        void subscribe_closing_duration(std::function<void(float)>&& f) { this->closingDuration.subscribe(std::move(f)); }
        void subscribe_openings(std::function<void(uint16_t)>&& f) { this->openings.subscribe(std::move(f)); }

        void subscribe_door_state(std::function<void(DoorState, float)>&& f) { 
            this->doorState.subscribe([=](DoorState state) {
                f(state, *this->doorPosition);
            });
            this->doorPosition.subscribe([=](float position) {
                f(*this->doorState, position);
            });
        }
        void subscribe_door_position(std::function<void(float)>&& f) { this->doorPosition.subscribe(std::move(f)); }
        void subscribe_light_state(std::function<void(LightState)>&& f) { this->lightState.subscribe(std::move(f)); }
        void subscribe_lock_state(std::function<void(LockState)>&& f) { this->lockState.subscribe(std::move(f)); }
        void subscribe_obstruction_state(std::function<void(ObstructionState)>&& f) { this->obstructionState.subscribe(std::move(f)); }
        void subscribe_motor_state(std::function<void(MotorState)>&& f) { this->motorState.subscribe(std::move(f)); }
        void subscribe_button_state(std::function<void(ButtonState)>&& f) { this->buttonState.subscribe(std::move(f)); }
        void subscribe_motion_state(std::function<void(MotionState)>&& f) { this->motionState.subscribe(std::move(f)); }
